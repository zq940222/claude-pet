...
| 优先级 | 输入 | 效果 |
| --- | --- | --- |
| 1 | 鼠标停在上面 | 展开。悬停窥视永远有效，任何时候碰一下都能看 |
| 2 | 用户点了钉住 | 展开，移开也不收 |
| 3 | 用户点了收起 | 收起，**压过下面那条** |
| 4 | 有会话在等你 | 展开 —— 要你确认或回话时自己亮出来 |

第 3 条压过第 4 条是有意的：你看见了、知道了，想先收起来接着干别的，就该收得掉。写成简单的「或」（waiting 直接返回展开）会让等待期间的点击完全失效——挂件赖着不走，而你没有任何办法。

但这个「收起」只管**当前这一批**等待事项：出现新的等待签名时 `pinned` 会被清回 null，所以下一件需要你的事照样弹出来。一次关闭不会让挂件从此哑掉。

**点击的含义是「我移开之后它还开着吗」。** 不能写成 `pinned = !expanded`：点击时鼠标必然在挂件上，而悬停优先级最高、`expanded` 恒为真，那样每次点击都只会得到「收起」，再也钉不住。所以判断的是移开之后的状态，这样每种情形都是一次点击就生效。
