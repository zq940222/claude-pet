...
- **默认摆到屏幕顶部居中 + 悬停窥视 + 拖到边缘入坞** —— 三条合起来是一套「停在顶上不挡路，要看碰一下」的交互。
  ...
  - 展开改成**按优先级**：悬停 > 钉住 > 明确收起 > 有人在等你。**要你确认或回话时自动展开**，鼠标移开也不收 —— 那会把挂件存在的意义藏起来
  - 点击的含义是**「我移开之后它还开着吗」**。不能写成 `pinned = !expanded`：点击时鼠标必然在挂件上，而悬停优先级最高、`expanded` 恒为真，那样每次点击都只得到「收起」，再也钉不住
...
- **等待期间点挂件收不起来** —— 上一版把展开写成简单的「或」且 `waiting` 排在最前面直接返回，于是 `pinned === false` 永远生效不了，`toggleExpanded` 也只在 `true` / `null` 之间切。结果是有会话在等你时挂件赖着不走、点它没有任何反应（这之前是能收的）。现在「明确收起」压过「有人在等你」，但只管当前这一批 —— 出现新的等待事项时 `pinned` 清回 null，下一件需要你的事照样弹出来，一次关闭不会让挂件从此哑掉。
