...
> Note: these five labels do not exist on the GitHub repo yet. `gh issue edit --add-label` fails on an unknown label rather than creating it, so create them once up front:
>
> ```bash
...
> ```
>
> `wontfix` usually already exists on a fresh GitHub repo — `gh label create` will error on it, which is harmless.
